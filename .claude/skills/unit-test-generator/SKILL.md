...
**核心原则**：
- **🚨 强制 subagent 派发（最高优先级硬约束）**：本 skill 的 UT 设计阶段（阶段 1）与所有批次代码生成（阶段 3）**必须通过 `Task` / `Agent` 工具显式派发独立 subagent 执行**，主上下文**严禁自行 Read 全部源码 / 自行 Write 设计文档 / 自行 Write 测试代码文件**。即使被测类很少也不得绕过——主上下文只做调度、回写、反思补发，**读源码、写设计文档、写测试代码全部交给 subagent**。判定违规的客观信号：本应派发 subagent 的步骤里出现了主上下文的 Read 源码正文 / Write 设计文档 / Write 测试文件等动作（除阶段 0 的小段 Read 与阶段 4.1 的回写 Edit 外）。一旦发现立即停下，改为派发 subagent 重做。**仅当被测类 ≤ 4 且场景极少**时（见阶段 3 缩放规则），允许主上下文直接生成；其余情况一律 subagent。
...
> **🚨 派发动作硬约束**：本节的"派发"必须是**真实的 `Task` / `Agent` 工具调用**——即在主上下文的本轮回复中出现一个 `Agent` 工具块，subagent_type=`general-purpose`，把下方 prompt 整体作为 `prompt` 参数传入。**严禁**主上下文自己 Read 全部源码、自己枚举测试场景、自己 Write 设计文档来"代替"这次派发。如果主上下文准备直接动手做这些，立即停下并改为派发 subagent。
...
> **🚨 派发动作硬约束（再次强调）**：本阶段每一个 batch **必须以一个真实的 `Task` / `Agent` 工具调用**形式发出，而**不是**主上下文自己 Read 设计文档全文 + 自己 Write 测试文件。同 wave 多 batch = 同一条消息里多个 `Agent` 工具调用块并排出现。**违规判定**：本阶段中主上下文出现 `Write` 测试文件、自己写测试方法体、或 `Read` 完整源码文件正文——立即停下，回退到本节重新以 subagent 形式派发。**例外**：3.4 的"按需 Read 设计文档测试类章节"与阶段 4.1 的"回写设计文档"是允许的主上下文动作；3.2 表中明确标记"主上下文直接生成"的场景（被测类 ≤ 4 且场景极少）也可豁免。

1. `Agent` 工具，subagent_type=`general-purpose`（**必须真实调用 `Agent` 工具**，而非主上下文文字描述"派发了 N 个 subagent"）
2. **按 wave 派发**：一次发整 wave 的全部 batch（5~8 并行）
3. **同消息内并行**：同 wave 所有 subagent 在同一条消息中发起多个 `Agent` 工具调用块
