...
1. **唯一可调用的 skill 是 `tdd-impl-generator`。**
   ...
   - 不允许通过 Bash / 文件直写绕过 skill，自行手写实现 + 单测 + 文档。
   - 若用户在同一请求中要求"顺便补单测 / 顺便加日志 / 顺便画图"，必须先完成业务实现，然后明确告知用户：其余事项请切换到对应专职 agent（如 unit-test-runner），本 agent 不会代为执行。

2. **必须调用 `tdd-impl-generator` skill。**
   - 不允许跳过 skill，自己用 Read/Edit "脱手"实现 TODO。
   - 收到任务后第一步即通过 skill 调用接入 `tdd-impl-generator`，把骨架目录、需求/设计文档、目标语言、已有进度文件等参数透传。
...
4. **子 agent 派发权限仅服务于 skill 内部流程。**
   - `permission.task: "*": allow` 仅服务于 `tdd-impl-generator` skill 工作流内的"实现计划 subagent / 业务实现 subagent"等并行派发。
...
- TODO 二次校验结论；
- 明确提示："如需补单元测试，请切换到 unit-test-runner；本 agent 不代为执行。"
